...
import { AlertCircle, Lock, User as UserIcon, BookOpen, ShieldAlert, ArrowRight } from 'lucide-react';
...
export function Login() {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      if (user.role === 'institution') {
        ...
        // Non-institution session detected (e.g. student or admin); clear session to allow branch login
        logout();
      }
    }
  }, [isAuthenticated, user, navigate, logout]);

  if (pendingMemberships.length > 0 && user?.role === 'institution') {
    navigate('/dashboard', { replace: true });
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4 py-8 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-emerald-600 rounded-full blur-[120px] opacity-15 pointer-events-none"></div>
      ...
      <Card className="w-full max-w-md bg-slate-800/80 border-slate-700/50 backdrop-blur-xl text-slate-100 shadow-2xl relative z-10 animate-fade-in">
        <CardHeader className="text-center pb-4">
          <div className="mx-auto h-20 w-auto bg-white p-2 rounded-2xl flex items-center justify-center mb-3 shadow-lg shadow-rose-500/20">
            <img src="/logo.png" alt="ASSISI IFL Logo" className="h-14 w-auto object-contain" />
          </div>
          <CardTitle className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-white via-emerald-200 to-emerald-400 bg-clip-text text-transparent">
            Institution Space
          </CardTitle>
          <p className="text-sm text-slate-400 mt-1">
            Assisi Social Campus Management Portal
          </p>
        </CardHeader>
        <CardContent>
          {serverError && (
          ...
          <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-4" noValidate>
            <div className="space-y-1.5">
              <Label htmlFor="username" className="text-slate-300">Campus Admin Username</Label>
              <div className="relative">
                <UserIcon className="absolute left-3.5 top-1/2 transform -translate-y-1/2 h-4 w-4 text-slate-400" />
                <Input
                  id="username"
                  type="text"
                  placeholder="assisivagamon"
                  className="pl-10 bg-slate-900/60 border-slate-700/60 focus:border-emerald-500 focus:ring-emerald-500/20 text-white"
                  autoFocus
            ...
            <Button
              type="submit"
              className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 py-2.5 font-bold shadow-lg shadow-emerald-600/15"
              disabled={loginForm.formState.isSubmitting}
            >
