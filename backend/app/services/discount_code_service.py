...
class DiscountCodeService(BaseService):
    ...
    def create_discount_code(
        ...
        # Check for duplicate
        existing = self.db.query(DiscountCode).filter(DiscountCode.code == code).first()
        ...
        self._log_action(
            "create_discount_code", "discount_code", discount_code.id, created_by
        )

        return discount_code

    def generate_discount_code(
        self, property_id: int, prefix: str = "PROMO", length: int = 8
    ) -> str:
        """
        ...
        while True:
            ...
            existing = (
                self.db.query(DiscountCode).filter(DiscountCode.code == code).first()
            )
    ...
    def validate_discount_code(
        ...
        discount = (
            ...
            .filter(DiscountCode.property_id == property_id, DiscountCode.code == code)
            .first()
        )
        ...
        if discount.valid_from and today < discount.valid_from:
            return {
                "valid": False,
                "error": f"Code not valid until {discount.valid_from}",
            }
        ...
        if num_nights < discount.min_nights:
            return {
                "valid": False,
                "error": f"Minimum {discount.min_nights} nights required",
            }

        if discount.min_amount and booking_amount < discount.min_amount:
            return {
                "valid": False,
                "error": f"Minimum booking amount ${discount.min_amount} required",
            }
        ...
        if guest_id and discount.max_uses_per_guest:
            ...
            guest_uses = (
                ...
                .filter(
                    Reservation.guest_id == guest_id, Reservation.discount_code == code
                )
                .count()
            )
        ...
        discount_amount = self._calculate_discount_amount(
            discount, booking_amount, num_nights
        )
    ...
    def apply_discount_code(
        self, discount_id: int, guest_id: Optional[int] = None
    ) -> DiscountCode:
        """
    ...
    def _calculate_discount_amount(
        self, discount: DiscountCode, booking_amount: Decimal, num_nights: int
    ) -> Decimal:
    ...
    def get_discount_codes(
        self, property_id: int, active_only: bool = True
    ) -> List[DiscountCode]:
    ...
    def deactivate_code(self, discount_id: int, deactivated_by: str) -> DiscountCode:
        ...
        self.refresh(discount)
        self._log_action(
            "deactivate_discount_code", "discount_code", discount_id, deactivated_by
        )
        return discount
